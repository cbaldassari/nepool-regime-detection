...
STRIDE_H      = 6
CHRONOS_MODEL = "amazon/chronos-2"          # multivariate, 120M params, D_model ~512
N_GPUS        = 3                           # Ray actors (one per GPU)
BATCH_SIZE    = 32                          # windows per batch per actor (lower = less GPU memory pressure)
MAX_RETRIES   = 3                           # Ray actor retries on node failure

FEAT_COLS = [
    "lmp", "log_lmp", "log_return", "total_mw",
...
def acf_analysis(out: pd.DataFrame, max_lag: int = 720) -> None:
    ...
    features    = ["log_return", "log_lmp", "total_mw", "ilr_7", "ilr_1"]
    feat_labels = ["log_return", "log_lmp", "total_mw",
    ...
    for ax, col in zip(axes, ["log_lmp", "total_mw", "ilr_7"]):
...
def embed_batch_chronos(pipeline, batch: np.ndarray, device: str) -> np.ndarray:
    ...
    # Tensor must stay on CPU — Chronos-2's DataLoader handles pin_memory
    # internally and will move it to the correct device automatically.
    x = torch.tensor(batch.transpose(0, 2, 1), dtype=torch.float32)
...
# ═══════════════════════════════════════════════════════════════════════════
#  RAY extraction  (Ray cluster — datalab-rayclnt.unitus.it)
# ═══════════════════════════════════════════════════════════════════════════
...
def extract_ray(windows: np.ndarray) -> np.ndarray:
    ...
    @ray.remote(num_gpus=1, max_restarts=MAX_RETRIES, max_task_retries=MAX_RETRIES)
    class ChronosActor:
        ...
        def embed_chunk(self, chunk: np.ndarray) -> np.ndarray:
            results = []
    ...
    actors  = [ChronosActor.remote(CHRONOS_MODEL) for _ in range(len(chunks))]
    futures = [a.embed_chunk.remote(c) for a, c in zip(actors, chunks)]

    print(f"\n  Dispatched {n} windows across {len(actors)} Ray actors")
    print(f"  Batch size per actor : {BATCH_SIZE}  |  max_retries : {MAX_RETRIES}")

    # Collect with ray.wait — processes results as they arrive,
    # tolerates node failures up to MAX_RETRIES per chunk.
    results  = {}   # future → result (keyed to preserve chunk order)
    pending  = list(futures)

    with tqdm(total=len(futures), desc="  Collecting", ncols=65,
    ...
    return np.concatenate([results[i] for i in range(len(futures))], axis=0)
...
def main() -> pd.DataFrame:
    ...
    steps = [
        ...
        "ACF / spectral analysis",
        "Build sliding windows",
        "Extract Chronos-2 embeddings",
        "Save embeddings",
        "Diagnostic plots",
    ]
    ...
    pbar.update(1)
    print(f"\n  Loaded {len(out):,} rows × {len(FEAT_COLS)} feature channels")

    # ── 2. ACF / spectral plots ──────────────────────────────────────────────
    # Confirms that 720h is the right context length by showing autocorrelation
    # is still meaningful at that lag, and that dominant cycles are 24h/168h/720h.
    ...
    # ── 3. Sliding windows ──────────────────────────────────────────────────
    # Each window is a contiguous 720h block of 11 features.
    # Windows crossing the 2023-06-15 gap (25h missing) are silently discarded.
    pbar.set_description(steps[2])
    windows, timestamps, n_skipped = build_windows(out)
    N = len(windows)
    pbar.update(1)

    print(f"\n  Windows built   : {N:,}")
    print(f"  Windows skipped : {n_skipped:,}  (gap / discontinuity)")
    print(f"  Date range      : {pd.Timestamp(timestamps[0]).date()}  →  "
          f"{pd.Timestamp(timestamps[-1]).date()}")
    print(f"  Window shape    : ({N}, {CONTEXT_LEN}, {len(FEAT_COLS)})")

    # ── 4. Embeddings ────────────────────────────────────────────────────────
    # Chronos-2 processes all 11 channels as one group via group attention.
    # Per window: embed(group) → (11, n_patches, D_model) → mean-pool patches
    # → (11, D_model) → flatten → (11 × D_model,) per window.
    ...
    print()
    print("  Next → Step 03: UMAP dimensionality reduction on embeddings")
    print("─" * 65)
