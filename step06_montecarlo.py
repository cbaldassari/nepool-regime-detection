...
def load_inputs() -> dict:
    ...
    for r in labels:
        ...
        if len(row) == 0 or not bool(row.iloc[0].get("valid", True)):
            ...
            ou_params[r] = {"theta": 0.0,
                            "mu":    float(np.arcsinh(stat_row["median_lmp"].values[0])),
                            "sigma": sigma_fb,
                            "valid": False}
        else:
    ...
    if INIT_LOG_LMP is not None:
        ...
        init_log_lmp = float(pre["arcsinh_lmp"].dropna().iloc[-1])

    print(f"  Initial arcsinh_lmp : {init_log_lmp:.4f}  "
          f"(LMP ≈ {np.sinh(init_log_lmp):.2f} $/MWh)")
...
def compute_summary(price_paths: np.ndarray) -> pd.DataFrame:
    ...
    lmp_paths = np.sinh(price_paths.astype(np.float64))
...
def plot_price_fan(summary: pd.DataFrame, init_log_lmp: float):
    ...
    init = float(np.sinh(init_log_lmp))
    ...
    axes[1].set_title("arcsinh-LMP forecast fan chart",  fontweight="bold")
    fig.suptitle(
...
def plot_terminal_distribution(price_paths: np.ndarray):
    ...
    """
    terminal_lmp = np.sinh(price_paths[:, -1].astype(np.float64))
...
def plot_sample_paths(price_paths: np.ndarray, n_show: int = 50, seed: int = 42):
    ...
    lmp_show = np.sinh(price_paths[idx].astype(np.float64))
    ...
    med = np.sinh(np.median(price_paths, axis=0))
...
def main():
    ...
    terminal    = np.sinh(price_paths[:, -1].astype(np.float64))
    ...
    print(f"  Trajectories       : {N_TRAJ:,}")
    print(f"  Horizon            : {HORIZON_H}h  ({HORIZON_H // 24} days)")
    print(f"  Initial LMP        : {np.sinh(inp['init_log_lmp']):.2f} $/MWh")
    print()
